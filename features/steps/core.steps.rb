...
Then(/^I can recover the message with the Random Key$/) do
  @contract.open_vault('message_locked_with_random').should == @msg
end
...
Then(/^another user can recover the message with the Unlocked Random Key$/) do
  ...
  @contract = VaultTree::Contract.new(@contract_json)
  @contract.open_vault('message_locked_with_unlocked_random_number').should == @msg
end
...
Then(/^they can create a DH Key and unlock the message$/) do
  @contract.open_vault('asymmetric_message').should == @asymmetric_message
end
...
Then(/^I can recover the message if each party gives consent$/) do
  ...
  @contract = VaultTree::Contract.new(@contract_json)
  @contract.open_vault('abc_consent_message', @unlocking_consent).should == @abc_consent_message
  puts @contract.open_vault('abc_consent_message')
end
...
Then(/^I cannot recover the message if one party fails to give consent$/) do
  ...
  @contract = VaultTree::Contract.new(@contract_json)
  expect{@contract.open_vault('abc_consent_message',@incomplete_unlocking_consent_keys)}.to raise_error(VaultTree::Exceptions::FailedUnlockAttempt)
end
